...
def affine_forward(x, w, b):
    ...
    x_reshaped = x.reshape(x.shape[0], np.prod(x.shape[1:])).astype(float)
...
def relu_backward(dout, cache):
    ...
    dx = dout * (np.maximum(0, x) != 0).astype(float)
...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == "train":
        ...
        inv_sqrt_x_var_plus_eps = 1/np.sqrt(x_var_plus_eps)
        x_mod = x_minus_mean * inv_sqrt_x_var_plus_eps
        out = gamma * x_mod + beta

        cache = (gamma, x_minus_mean, inv_sqrt_x_var_plus_eps, x_mod)
...
def batchnorm_backward(dout, cache, compute_dx_only=0):
    ...
    (gamma, x_minus_mean, inv_sqrt_x_var_plus_eps, x_mod) = cache
    N = dout.shape[0]

    dx_mod = dout * gamma
    dvar = np.sum(dx_mod * x_minus_mean * (-1/2) * inv_sqrt_x_var_plus_eps**3, axis=0)
...
def batchnorm_backward_alt(dout, cache):
    ...
    (dx, dgamma, dbeta) = batchnorm_backward(dout, cache) # is already optimized
...
def conv2d(A, mask, stride, pad=0):
    [H, W] = A.shape
    [HH, WW] = mask.shape
    ...
    for h_ in range(H_):
        for w_ in range(W_):
            h = h_*stride
            w = w_*stride
            out[h_, w_] = np.sum(A_pad[h:h+HH, w:w+WW]*mask)
    return out


def maxpool2d(A, pool_height, pool_width, stride):
    [H, W] = A.shape
    ...
    for h_ in range(H_):
        for w_ in range(W_):
            h = h_*stride
            w = w_*stride
            out[h_, w_] = np.max(A[h:h+pool_height, w:w+pool_width])
    return out
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    for n in range(N):
        for f in range(F):
            temp = np.zeros((H_, W_))
            for c in range(C):
                temp += conv2d(x[n,c], w[f,c], stride, pad)
            out[n, f] = temp + b[f]
...
def conv_backward_naive(dout, cache):
    ...
    npad = ((0, 0), (0, 0), (pad, pad), (pad, pad))  # pad around x's 3rd and 4th dimensions
...
def max_pool_forward_naive(x, pool_param):
    ...
    for n in range(N):
        for c in range(C):
            out[n, c] = maxpool2d(x[n, c], pool_height, pool_width, stride)
        ...
        for c in range(C):
            ...
            for h_ in range(H_):
                for w_ in range(W_):
                    h = h_*stride
                    w = w_*stride
                    (a, b) = np.unravel_index(np.argmax(
                        X[h:h+pool_height, w:w+pool_width]),
                        (pool_height, pool_width))
                    dx[n, c, h+a, w+b] = dout[n, c, h_, w_]
...
def spatial_batchnorm_forward(x, gamma, beta, bn_param):
    ...
    (N,C,H,W) = x.shape
    x_flat = np.reshape(np.transpose(x, (0,2,3,1)), (N*H*W, C))
    out_flat, cache = batchnorm_forward(x_flat, gamma, beta, bn_param)
    out = np.transpose(np.reshape(out_flat, (N,H,W,C)), (0,3,1,2))
...
def spatial_batchnorm_backward(dout, cache):
    ...
    (N,C,H,W) = dout.shape
    dout_flat = np.reshape(np.transpose(dout, (0,2,3,1)), (N*H*W, C))
    dx_flat, dgamma, dbeta = batchnorm_backward(dout_flat, cache)
    dx = np.transpose(np.reshape(dx_flat, (N,H,W,C)), (0,3,1,2))
...
def spatial_groupnorm_forward(x, gamma, beta, G, gn_param):
    ...
    N, C, H, W = x.shape
    x_res = np.resize(x, (N, G, C//G, H , W))
    mean = np.mean(x_res, axis=(2,3,4), keepdims=True)
    var = np.var(x_res, axis=(2,3,4), keepdims=True)
    x_mod = (x_res - mean)/np.sqrt(var + eps)
    x_norm = np.reshape(x_mod, (N,C,H,W))
